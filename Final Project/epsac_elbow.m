...
ueq = M*g*sin(theta_eq)*h + m*g*sin(theta_eq)*h/2 + K*theta_eq; % input needed to keep the initial position
...
for k = 1:num
    ...
    x(k) = 2*y1 - y2 + (Ts^2/Jeq)*(-M*g*h*sin(y2) - m*g*h*sin(y2)/2 - (b/Ts)*(y1 - y2) - K*y2 + u2);
    n(k) = y(k) - x(k);
    ...
    for i =1:N
        ...
        % free response
        x1(i) = 2*x11 - x12 + (Ts^2/Jeq)*(-M*g*h*sin(x12) - m*g*h*sin(x12)/2 - (b/Ts)*(x11 - x12) - K*x12 + u1);
        y11(i) = x1(i) + n(i);
        x12 = x11;
        x11 = y11(i);
        
        % step response
        x2(i) = 2*x21 - x22 + (Ts^2/Jeq)*(-M*g*h*sin(x22) - m*g*h*sin(x22)/2 - (b/Ts)*(x21 - x22) - K*x22 + u1 + a);
        y22(i) = x2(i) + n(i);
        x22 = x21;
        x21 = y22(i);
    ...
    if k >= 100
        ...
        y(k+1) = 2*y1 - y2 + (Ts^2/Jeq)*(-M*1.5*g*h*sin(y2) - m*g*h*sin(y2)/2 - (b/Ts)*(y1 - y2) - K*y2 + u1 - 0.5);
    else
        y(k+1) = 2*y1 - y2 + (Ts^2/Jeq)*(-M*1.5*g*h*sin(y2) - m*g*h*sin(y2)/2 - (b/Ts)*(y1 - y2) - K*y2 + u1);
    end
    
    % noise
    if k>= 200
        y(k+1) = y(k+1) + 10^-2*rand;
    end
    %%
